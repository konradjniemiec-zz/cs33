...
{
  int i, j;
  int index_X, index_Y,hoistArrayX, hoistArrayY;
  int max_size = X*Y*Z;
#pragma omp parallel for private (j, index_X, index_Y,hoistArrayX,hoistArrayY)
  for(i = 0; i<n; i++){
    arrayX[i] += 1 + 5*rand2(seed, i);
    arrayY[i] += -2 + 2*rand2(seed, i);
    hoistArrayX = arrayX[i];
    hoistArrayY = arrayY[i];
    probability[i] = 0;
    for(j = 0; j < Ones; j++){
      index_X = round(hoistArrayX) + objxy[j*2 + 1];
      index_Y = round(hoistArrayY) + objxy[j*2];
